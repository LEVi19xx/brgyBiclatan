...
//------------------------------------------ DATABASE CONNECTION ----------------------------------------------------
    ...
    protected $server = "mysql:host=localhost;dbname=u792431185_bmis";
    protected $user = "u792431185_biclatan";
    protected $pass = "brgyBiclatan24";
...
public function create_travelpermit() {
    ...
    public function create_brgyclearance_walkin() {
        ...
        if(isset($_POST['create_brgyclearance_walkin'])) {
            ...
            $stmt->execute([$lname, $fname, $mi,  $purpose, 
    ...
    public function create_bspermit() {
        if(isset($_POST['create_bspermit'])) {
            $id_bspermit = uniqid(); // Generate a unique identifier for id_bspermit
    
            // Retrieve other form fields from $_POST array
            $id_resident = $_POST['id_resident'];
            $lname = $_POST['lname'];
            $fname = $_POST['fname'];
            $mi = $_POST['mi'];
...
public function create_blotter_walkin() {
        ...
        if(isset($_POST['create_blotter_walkin'])) {
            ...
            $connection = $this->openConn();
            $stmt = $connection->prepare("INSERT INTO tbl_blotter (`lname`, `fname`, `mi`,
            `houseno`, `street`,`brgy`, `municipal`, `contact`, `narrative`)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

            $stmt->execute([$lname, $fname, $mi, $houseno,  $street, $brgy, $municipal, $contact, $narrative]);
